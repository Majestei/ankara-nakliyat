...
export default function GoogleReviewsClient({ initialData }: Props) {
    ...
    return (
        <div className="space-y-12">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 sm:p-12 border border-slate-200/80 shadow-xl relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row gap-8 lg:items-center justify-between">
                    ...
                    <div className="space-y-4">
                        <div className="inline-flex items-center gap-2.5 px-4 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-xs font-bold text-slate-700">
                        ...
                        <div className="flex items-baseline gap-5">
                            <span className="text-5xl sm:text-7xl font-black text-slate-900 tracking-tighter">
                                {data.rating.toFixed(1)}
                            </span>
                            <div>
                                <div className="flex text-amber-400 gap-1 text-2xl sm:text-3xl">
                                    {[...Array(5)].map((_, i) => (
                                        <span key={i}>★</span>
                                    ))}
                                </div>
                    ...
                    {/* Middle: Rating Bars */}
                    <div className="w-full lg:w-72 space-y-2.5 border-t lg:border-t-0 lg:border-l border-slate-200 pt-6 lg:pt-0 lg:pl-8">
                        <div className="flex items-center gap-3 text-xs font-semibold text-slate-600">
                            <span className="w-14">5 Yıldız</span>
                            <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                                <div className="h-full bg-gradient-to-r from-amber-400 to-orange-400 rounded-full" style={{ width: `${fiveStarPercent}%` }} />
                            </div>
                            <span className="w-8 text-right font-bold text-slate-900">{fiveStarPercent}%</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs font-semibold text-slate-600">
                            <span className="w-14">4 Yıldız</span>
                            <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                        ...
                        <div className="flex items-center gap-3 text-xs font-semibold text-slate-600">
                            <span className="w-14">3 Yıldız</span>
                            <div className="flex-1 h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                    ...
                    <div className="flex flex-col sm:flex-row lg:flex-col gap-3 shrink-0">
                        <a
                            ...
                            className="btn-primary !py-4 !px-8 rounded-full flex items-center justify-center gap-2.5 text-xs font-black uppercase tracking-wider shadow-lg shadow-primary-500/25"
                        ...
                        <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center justify-center gap-2 px-7 py-3.5 rounded-full border border-slate-300 bg-white text-xs font-black uppercase tracking-wider text-slate-700 hover:bg-slate-50 transition-all shadow-sm"
                ...
                <div className="mt-8 pt-4 border-t border-slate-100 flex flex-wrap items-center justify-between gap-3 text-xs text-slate-500">
                    ...
                    <button
                        ...
                        className="text-primary-600 hover:text-primary-700 font-semibold underline text-xs cursor-pointer"
                    >
            ...
            {showApiGuide && (
                <div className="bg-slate-900 text-slate-200 rounded-[2.5rem] p-8 sm:p-10 border border-slate-800 text-sm space-y-4">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                        ...
                        <button
                            ...
                            className="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-full bg-slate-800 cursor-pointer"
                        >
                            Kapat
                        </button>
                    </div>
                    ...
                    <div className="bg-black/60 p-4 rounded-2xl font-mono text-xs text-green-400 border border-slate-800 overflow-x-auto space-y-1">
            ...
            {/* ── FILTER & SEARCH BAR ── */}
            <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center bg-white p-5 rounded-[2rem] border border-slate-200/80 shadow-md">
                {/* Rating Filter Tabs */}
                <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0">
                    <button
                        onClick={() => setFilterRating(null)}
                        className={`px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider transition-all shrink-0 ${
                            filterRating === null
                    ...
                    <button
                        ...
                        className={`px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-wider transition-all shrink-0 flex items-center gap-1.5 ${
                            filterRating === 5
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <select
                        ...
                        className="bg-slate-100 border border-slate-200 text-xs font-bold text-slate-700 rounded-full px-5 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500"
                    >
                    ...
                    <input
                        ...
                        className="bg-slate-100 border border-slate-200 text-xs text-slate-800 rounded-full px-5 py-2.5 focus:outline-none focus:ring-2 focus:ring-primary-500 w-full sm:w-64"
                    />
                </div>
            </div>
            ...
            {filteredReviews.length === 0 ? (
                <div className="bg-white rounded-[2.5rem] p-12 text-center border border-slate-200 text-slate-500">
                    ...
                    <button
                        ...
                        className="btn-outline !py-2.5 !px-6 rounded-full text-xs font-bold"
                    >
                        Filtreleri Temizle
                    </button>
                </div>
                    ...
                    {filteredReviews.map((review) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="flex items-center gap-3.5">
                                            <div className={`w-11 h-11 rounded-2xl bg-gradient-to-br ${review.avatar_color} text-white font-black flex items-center justify-center text-sm shadow-md shrink-0`}>
                                                {initial}
                                            </div>
                                            <div>
                                ...
                                {review.owner_reply && (
                                    <div className="mt-5 pt-4 border-t border-slate-100 bg-slate-50/80 -mx-8 -mb-8 p-6 rounded-b-[2.5rem]">
                                        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-800 mb-1.5">
                                            <span className="w-2 h-2 rounded-full bg-primary-600" />
                                            <span>Ankara Özdemir Nakliyat (İşletme Yanıtı)</span>
            ...
            <div className="bg-white rounded-[2.5rem] p-8 sm:p-10 border border-slate-200/80 shadow-xl">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
                    ...
                    <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="btn-outline !py-3 !px-6 rounded-full text-xs font-bold shrink-0 flex items-center gap-2 shadow-sm"
                ...
                <div className="w-full h-72 sm:h-96 rounded-[2rem] overflow-hidden border border-slate-200 shadow-inner">
                    <iframe
